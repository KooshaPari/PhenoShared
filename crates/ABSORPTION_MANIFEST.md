...
Files under `crates/` from archived `<REDACTED>/PhenoProc` (wave 6, 2026-06-17).
...
Rust LLM router from `<REDACTED>/phenoRouterMonitor` is canonical in
`<REDACTED>/phenoAI/crates/llm-router/` — `LlmProvider` trait, `OpenAiProvider`,
...
+ 8 smoke tests live in `<REDACTED>/phenoAI/ports/`. Source repo was archived
...
from `<REDACTED>/agent-platform` is canonical in
...
KlipDot absorption target is `<REDACTED>/rich-cli-kit/crates/klipdot/`
...
3. `agileplus-*` → **canonical in `<REDACTED>/AgilePlus`** (staging copies removed from Agentora 2026-06-17)
