...
//!
//! Reference pattern from Apisync's hexagonal `src/domain/mod.rs`. This file
//! is documentation/reference-quality and is intentionally NOT compiled as
//! part of the HexaKit library crate (it depends on `async_trait` and serde
//! types that do not exist in HexaKit).
//!
//! The pattern shows how a hexagonal `domain` module exposes:
//! - transport-agnostic `Request` / `Response` types
//! - an `Endpoint` trait that concrete adapters implement
//! - a simple in-memory `ItemStore` for unit testing
//! - unit tests for each type, demonstrating how domain logic can be tested
//!   without spinning up an HTTP server (the hexagonal testability win).
