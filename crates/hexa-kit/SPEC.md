# HexaKit - Specification

## Problem

`HexaKit` addresses a specific need in the Phenotype fleet: phenotype-org hexagonal architecture toolkit. Without a canonical implementation, downstream consumers must reinvent the same primitives, leading to fragmentation and divergent behavior across the fleet.

## Solution

`HexaKit` provides a single, well-tested, well-documented implementation of this capability. The package ships with:
- A stable public API
