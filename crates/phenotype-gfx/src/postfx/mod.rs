...
//! - [`ports`] — hexagonal port traits + adapters (material registry, serialization, shader
//!   availability, LUT pipeline, URP render-graph).
...
//! - [`post_stack`] — engine-agnostic config + availability + quality; `describe_passes()` returns
//!   `Vec<PassDescriptor>`.
//! - [`rendering`] — engine-agnostic render-target + material handle types (used by the C# edge;
//!   `PostFxMaterial` is a `#[deprecated]` pass-through).
