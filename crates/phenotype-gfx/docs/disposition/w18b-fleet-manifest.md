...
- **W18b pheno gate:** manifest scan on `main` — no `<REDACTED>/pheno` or `phenoShared` git pins in `Cargo.toml` / `go.mod`.
...
| Area | Before | After |
|------|--------|-------|
| `VERSION.toml` module upstream comments | `<REDACTED>/phenotype-voxel` etc. | in-repo `src/voxel`, `unity/terrain`, `unity/water` |
| `unity/water` CI | checkout `<REDACTED>/phenotype-terrain` | in-monorepo `../terrain` project reference |
