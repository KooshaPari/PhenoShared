...
|---|---|---|---|
| `FocalPoint` | Yes (507 B) | Appended ADR-023 soft-block comment via PR | **PR #140 OPEN** `https://github.com/<REDACTED>/FocalPoint/pull/140` |
...
**Verification:** `gh api repos/<REDACTED>/<repo>` + `--jq '.archived'` per repo, 2026-06-20.
**Script:** `scripts/batch_codeowners_prs.sh` updated to handle archived/404 repos gracefully.

## FU3 execution log

```
$ gh pr view 140 --repo <REDACTED>/FocalPoint --json title,state,url
{
  "state": "OPEN",
  "title": "docs(governance): add ADR-023 PAUSED soft-block to CODEOWNERS (2026-06-20)",
  "url": "https://github.com/<REDACTED>/FocalPoint/pull/140"
}
