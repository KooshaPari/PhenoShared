...
# Dispatch
gh --version && gh auth status                        # GitHub CLI (<REDACTED> active as of 2026-06-17 22:30 PDT)
...
| **ADR-030** | **pheno-worklog-schema v2.1 — add 11th `device:` column (macbook / heavy-runner / subagent / ci)** | **L5-104.5, 2026-06-17** — see `pheno-worklog-schema/SPEC-v2.1.md`; PR `<REDACTED>/pheno-worklog-schema#1` open; 30/30 tests; 4 fleet WORKLOG.md migrated; v2.0 deprecation **2026-06-22** |
...
| **ADR-036** **[CLOSED 2026-06-19]** | **pheno-capacity substrate canonical** | L5-106, 2026-06-18 — see `docs/adr/2026-06-18/ADR-036-pheno-capacity.md`; extracted from `HwLedger` (per drift-detector retroactive hit) → **EXECUTED 2026-06-19**; `pheno-capacity` repo created; `<REDACTED>/pheno-capacity#1` merged; registry row added; `bucket_change HwLedger: from=CONDITIONAL to=STABLE reason=pheno-capacity extracted as canonical substrate` |
...
| **ADR-047** | **Predictive DRY discipline (4-criterion rule)** | L5-112, 2026-06-18 — see `docs/adr/2026-06-18/ADR-047-predictive-dry.md` + [§ Predictive DRY](#predictive-dry-adr-047) below; tool: `<REDACTED>/pheno-predict` (L72) |
| **ADR-048** | **Substrate graduation path (4-tier gate table)** | L5-113, 2026-06-18 — see `docs/adr/2026-06-18/ADR-048-substrate-graduation-path.md` + [§ Substrate graduation path](#substrate-graduation-path-adr-048) below; tool: `<REDACTED>/pheno-framework-lint` (L73) |
| **ADR-049** | **App-substrate drift detector (3-pass algorithm)** | L5-114, 2026-06-18 — see `docs/adr/2026-06-18/ADR-049-app-substrate-drift-detector.md` + [§ App-substrate drift detector](#app-substrate-drift-detector-adr-049) below; tool: `<REDACTED>/pheno-drift-detector` (L74) |
...
- **argis PR #108 created** — cross-repo coordination PR pinning the spike to `<REDACTED>/argis` for downstream consumers.
...
| :------------- | :---------------------------------- | :--------------------------------------------------------------------------------- |
| `AtomsBot*`    | PAUSED (capstone, legally mined)    | `<REDACTED>/AtomsBot:wip/2026-06-17-pre-pause-snapshot` @ `de10237`                |
| `focalpoint`   | PAUSED                              | `<REDACTED>/FocalPoint:main` @ `3ae2f126` (1 commit ahead)                         |
| `Dino`         | PAUSED (was CONDITIONAL)            | (no unpushed)                                                                     |
| `QuadSGM`      | PAUSED                              | `<REDACTED>/QuadSGM:wip/2026-06-17-pre-pause-snapshot` @ `484dfa1`                 |
| `HwLedger`     | PAUSED (default per ADR-023 Rule 3) | `<REDACTED>/HwLedger:wip/2026-06-17-cleanup-hwLedger` @ `f031f36`                  |
...
- `<REDACTED>/Configra` exists (created 2026-03-25). It is the real repo to absorb.
...
- Outstanding follow-ups: 5 stale `<REDACTED>/Pyron:README.md` and `<REDACTED>/phenotype-registry:*` link references need re-pointing to `<REDACTED>/phenotype-apps` ADR paths (non-blocking, cosmetic)

**Historical state (pre-closure):**
- `<REDACTED>/phenotype-monorepo-state` existed (created 2026-06-18 03:52 UTC) — held 4 governance-snapshot commits.
...
- **dispatch-mcp deletion vs archive**: User said "dispatch-mcp should be deleted" but `gh repo delete` requires `delete_repo` scope on Dmouse92 token (HTTP 403, current scopes: `'gist', 'read:org', 'repo', 'workflow'`). Archive is the only available action. `Dmouse92/dispatch-mcp` is archived (not deleted); the consumer-facing work is fully absorbed into `<REDACTED>/pheno-mcp-router` and `<REDACTED>/dispatch-mcp` per the 6 Track 8 PRs.
- **L5-104 MIGRATION GUARANTEE VERIFIED (2026-06-17 22:15 PDT)**: Orchestrator-level shell verification confirms 100% migration coverage:
  ...
  - **Aggregate**: 0 net content loss; audit doc `findings/2026-06-17-L5-104-dmouse92-to-<REDACTED>.md` §4.5
- **4-repo retirement (L5-109..114) COMPLETE 2026-06-18**: phenotype-voxel, phenotype-terrain, phenotype-water, phenotype-postfx all archived+deleted. Registry entries flipped to terminal fsm=archived. PRs: <REDACTED>/phenotype-gfx#10 (migration), #11 (audit sync); <REDACTED>/phenotype-registry#200/#203 (pre-archive + post-archive).
...
- **PROCESS-KILL GOVERNANCE (recorded 2026-06-22 per user directive)**:
  ...
  - Applies to ALL non-owned repos (anything outside `<REDACTED>/*`). Do not extend `kill` semantics to fix tool-availability issues — the tool stack is sacred and recovers from lock conditions on its own.
