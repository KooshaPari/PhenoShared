...
impl ContextFoldHandle {
    ...
    pub fn fold(&self, payload: &str) -> (String, usize) {
        ...
        // Increment the fold counter inside its own scope so the
        // MutexGuard is dropped *before* the tracing call below;
        // holding a lock through logging can block the hot path.
        {
            let mut count = self.fold_count.lock().expect("fold_count mutex poisoned");
            *count += 1;
        }
        ::tracing::info!(
            before = before,
...
mod tests {
    ...
    /// Regression: the fold_count MutexGuard must be dropped *before*
    /// the tracing::info! call.  If the guard leaks into the logging
    /// path, concurrent folds on the same handle can deadlock.  This
    /// test spawns N threads folding in parallel and verifies the count
    /// is correct and no thread panics (deadlock surfaces as a
        ...
        for _ in 0..n_threads {
            ...
            joins.push(thread::spawn(move || {
                for _ in 0..folds_per_thread {
                    let _ = h.fold("payload");
                }
            }));
        }

        for j in joins {
            j.join().expect("thread must not deadlock or panic");
        }

        assert_eq!(
            handle.fold_count(),
            (n_threads * folds_per_thread) as u64,
            "fold_count must reflect every completed fold"
        );
    }
}
