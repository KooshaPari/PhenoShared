        ...
        let decision = match self.lookup(req) {
            ...
            Some(transform) => match transform.apply(req) {
                Ok(rewritten) => {
                    ...
                    // Do NOT set chosen_model here: rewritten.id is the
                    // request routing identifier (e.g. "identity:user:42"),
                    // not a model name.  Only concrete model selectors
                    // (e.g. intelligentrouter) should populate chosen_model.
                    d.annotations.push((
...
mod tests {
    ...
    fn prompt_adapter_rewrites_via_identity_transform() {
        ...
        assert!(
            ...
            "promptadapter must not populate chosen_model from request id"
        );
        let ann = d
            .annotations
            .iter()
    ...
    /// Regression: promptadapter must NOT populate chosen_model from
    /// the request id.  The request id is a routing identifier
    /// (e.g. "identity:user:42"), not a model name.  Only concrete
    /// model selectors (e.g. intelligentrouter) should set
    /// chosen_model.  This test verifies that a successful transform
    /// leaves chosen_model as None and records the transform name in
    /// annotations instead.
    #[test]
    fn chosen_model_stays_none_after_successful_transform() {
        let pa = PromptAdapter::with_defaults();
        let d = pa
            .apply(&Request::new("identity:user:99", "test payload"))
            .expect("apply must succeed");
        assert!(matches!(d.decision, Decision::Allow));
        ...
        assert_eq!(
            transform_ann,
            Some("identity"),
            "transform name must be in annotations"
        );
    }
}
