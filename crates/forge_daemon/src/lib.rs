...
/// Safe wrapper around the FFI `forge_daemon_is_running()` probe.
/// Returns `false` if the daemon hasn't been started in this process or the
/// FFI call fails. Used by callers (e.g. `engine-forge::run_simple`) to decide
/// whether to take the daemon fast path or fall back to direct `Command::spawn`.
pub fn ffi_is_running() -> bool {
    unsafe { forge_daemon_is_running() != 0 }
}

/// Safe wrapper around the FFI `forge_daemon_start()` call.
/// `socket_path` may be `None` to use the default `/tmp/forge-daemon-<uid>.sock`.
/// Returns `Ok(())` on success, `Err(String)` on FFI failure.
///
/// Calling this twice is safe — the second call is a no-op if the daemon is
/// already running.
pub fn ffi_start(socket_path: Option<&str>) -> Result<(), String> {
    let c_path = match socket_path {
        Some(p) => CString::new(p).map_err(|e| format!("socket path NUL: {e}"))?,
        None => CString::new("").unwrap(),
    };
    let ptr = if socket_path.is_some() {
        c_path.as_ptr()
    } else {
        std::ptr::null()
    };
    let rc = unsafe { forge_daemon_start(ptr) };
    if rc == 0 {
        Ok(())
    } else {
        Err(format!("forge_daemon_start returned {rc}"))
    }
}

/// Safe wrapper around the FFI `forge_daemon_stop()` call.
pub fn ffi_stop() {
