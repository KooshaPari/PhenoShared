...
//!
//! - `replan` subcommand reads a `ReplanRequest` from `--request <file>` or stdin and prints a
//!   `ReplanResponse` (or `ReplanErrorResponse`) to stdout.
//! - Errors emit `ReplanErrorResponse` on stdout (so callers can parse it uniformly) and use the
//!   numeric exit code from `ReplanErrorCode::exit_code()`.
