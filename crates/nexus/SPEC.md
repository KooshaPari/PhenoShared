...
`nexus` addresses a specific need in the Phenotype fleet: service registry and discovery for microservices. Without a canonical implementation, downstream consumers must reinvent the same primitives, leading to fragmentation and divergent behavior across the fleet.

## Solution

`nexus` provides a single, well-tested, well-documented implementation of this capability. The package ships with:
- A stable public API
...
- Language: rust
- Tier: 2 (0=foundational, 1=core, 2=extension, 3=experimental)
- Maturity: stable
