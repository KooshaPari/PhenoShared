...
use crate::argis_monitor::alerts::payload::AlertPayload;
    ...
    match tracker.state {
        ...
        | AlertState::Pending {
            ...
            if burn >= threshold {
                if rule.for_secs.is_zero() {
                    ...
                    tracker.state = AlertState::Firing {
                        since: ts,
                        last_fired_at: ts,
                    };
        ...
        AlertState::Firing {
            ...
            if burn < resolve {
                ...
                let payload =
                    AlertPayload::resolved(&rule.name, target_name, &rule.slo, burn, resolve, ts);
                tracker.state = AlertState::Ok;
                tracker.sustained_for = Duration::from_secs(0);
                Decision::Fire(payload)
                ...
                let payload =
                    AlertPayload::firing(&rule.name, target_name, &rule.slo, burn, threshold, ts);
                tracker.state = AlertState::Firing {
                    since,
                    last_fired_at: ts,
                };
