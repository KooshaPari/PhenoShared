...
impl<T: Hash + Eq> DedupFilter<T> {
    /// Create an empty deduplication filter.
    pub fn new() -> Self {
        Self {
            seen: HashSet::new(),
        }
    ...
    /// Create an empty deduplication filter pre-allocated to `capacity` items.
    ...
    /// Insert `item` and return `true` if it was not already present.
    ///
    /// Returns `false` when the item was already in the set.
    pub fn check_and_insert(&mut self, item: T) -> bool {
        self.seen.insert(item)
    }

    /// Remove all previously recorded items.
    pub fn clear(&mut self) {
        self.seen.clear();
    }

    /// Return the number of distinct items recorded so far.
...
impl BloomFilter {
    /// Create a new bloom filter with `size` bits and `hash_count` hash functions.
    ...
    ///
    /// After calling this method, [`BloomFilter::check`] will likely return
    /// `true` for the same item (subject to the false-positive rate).
    ...
    /// Check whether the item is present, and insert it if not.
    ///
...
pub enum LockStatus {
    /// The lock is actively held by a process.
    Locked,
    /// The lock has been explicitly released.
    Released,
    /// The lock has exceeded its TTL and is no longer valid.
    Expired,
}
