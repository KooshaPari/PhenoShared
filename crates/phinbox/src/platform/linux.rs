    ...
    let outcome = match &spec.field {
        ...
        FieldSpec::Choice {
            ...
            for (i, o) in options.iter().enumerate() {
                cmd.arg(if default_index == &Some(i) {
                    "TRUE"
                } else {
                    "FALSE"
                });
...
fn render_kdialog(spec: &PromptSpec, opts: &ElicitOptions) -> Result<ElicitResponse, ElicitError> {
    ...
    let outcome = match &spec.field {
        ...
        FieldSpec::Choice {
            ...
            cmd.arg("Pick");
            for (i, o) in options.iter().enumerate() {
                let state = if default_index == &Some(i) {
                    "on"
                } else {
                    "off"
                };
...
fn render_tkinter(spec: &PromptSpec, opts: &ElicitOptions) -> Result<ElicitResponse, ElicitError> {
