...
use super::super::{
    css::full_html_css,
    helpers::{field_kind_label, format_age, html_escape, unix_now_ms_diff, NAV_HTML},
};
...
pub fn render_expired_html(req: &PendingRequest) -> String {
    ...
    format!(
        "<!doctype html><html lang=en><meta charset=utf-8><meta name=viewport \
         content='width=device-width,initial-scale=1'><title>Expired: \
         {title}</title><style>{css}</style><body>{nav}<div class=card expired{urg}><div \
         class=row><div class=row-main><strong>{title}</strong><span \
         class=ago>{ago}</span></div><div class=row-sub><span>{field_kind}</span><span \
         class=badge>Expired</span></div></div></div><main class=card><h2>{question}</h2><p \
         class=expired-notice>This request has expired and can no longer be answered.</p><a \
         href=/inbox class=ok>Return to inbox</a></main></body></html>",
