...
//! - `notify_node_failure` returns the list of invalidations — the caller is responsible for
//!   external notification (event log emission, Go-side handle drop).
//! - `bind_with_topology` (in `surface_ops`) replaces the `derive_endpoint_for_step` placeholder
//!   with a real topology lookup.
