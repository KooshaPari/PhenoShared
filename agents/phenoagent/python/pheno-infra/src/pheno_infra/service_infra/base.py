"""
Base ServiceInfra initialization and lifecycle helpers (refactored from kinfra/base.py).
"""
...
class BaseServiceInfra:
    ...
    def __init__(self, domain: str = "<REDACTED>.com", config_dir: str | None = None) -> None:
        ...
        if not getattr(BaseServiceInfra, "_global_cleanup_registered", False):
            atexit.register(self._cleanup_on_exit)
            signal.signal(signal.SIGTERM, self._cleanup_on_signal)
            signal.signal(signal.SIGINT, self._cleanup_on_signal)
            BaseServiceInfra._global_cleanup_registered = True
            logger.debug("Registered ServiceInfra cleanup handlers")

        # Clean up any stale processes from previous runs
        self._cleanup_stale_processes()

        logger.info(f"ServiceInfra initialized for domain: {domain}")
    ...
    def cleanup_all(self) -> None:
        ...
        """
        logger.info("Cleaning up all ServiceInfra resources")
        ...
        logger.info("ServiceInfra cleanup completed")
    ...
    def cleanup_environment(
        ...
        """
        logger.info("Running ServiceInfra runtime environment cleanup")
