# L5-104.x — Bulk Dmouse92 → <REDACTED> HEAD-vs-HEAD Migration Plan (14 Rust/TS/Python repos)

**Task ID:** L5-104.x (sub-id of L5-104 Dmouse92→<REDACTED> audit, "subagent D" lane per parent audit §4)
**Date:** 2026-06-17
...
- **PhenoProc + Nanovms caveat:** KP-side is `archived=true` per `gh api repos/<REDACTED>/PhenoProc` and `repos/<REDACTED>/nanovms` (verified 2026-06-17). DM92 mirrors are empty, so the only outstanding housekeeping is to archive DM92 to stop it from showing up in searches.
...
**KP state:** `archived=true` per `gh api repos/<REDACTED>/PhenoProc` (verified 2026-06-17 19:24 PDT, `pushed_at=2026-06-13T01:23:31Z`).
...
**Verdict:** Nothing to migrate. Both sides retired. **Action: `gh repo archive Dmouse92/PhenoProc --confirm` with note in DM92 description: "Merged into <REDACTED>/PhenoProc (now archived) — see <REDACTED> governance repo for the canonical phenotype-processor substrate."**
...
**Open PRs on DM92:** 0. **Open PRs on KP:** see parent audit §1.1 row 15 — "0 branches ahead" per `gh api repos/<REDACTED>/KWatch/branches?per_page=100` returns 31+ branches (all KP-side work, none in DM92 mirror).
...
**Verdict:** Nothing to migrate. Both sides retired. **Action: `gh repo archive Dmouse92/Nanovms --confirm` with note: "Merged into <REDACTED>/nanovms (now archived). Three-tier VM isolation concept lives on in ADR-023 substrate placement discussion; no active substrate candidate yet."**
...
**Top KP-ahead commits:** `4c7ebf1 feat(repo) add missing contributor docs`, `673d364 chore(ci) add SLSA + dependabot ecosystem + .gitignore + port scaffold`, `1d7a220 chore adopt dprint`, `4730d9c ci add OSSF Scorecard supply-chain security workflow`, `ee0e699 docs(changelog) add CHANGELOG.md with [Unreleased] section`, `161ad8d chore(governance) add CODEOWNERS with @<REDACTED> as default owner`, `507d36e chore(governance) add CODE_OF_CONDUCT.md,CONTRIBUTING.md,SECURITY.md,LICENSE`, `4b96887 Add ISSUE_TEMPLATE bug, feature, security, question + chooser config`, `1f72ff6 chore(justfile) standardize with build/test/lint/fmt/audit/deny/grade/ci`, `dc754c2 ci: add comprehensive PULL_REQUEST_TEMPLATE.md`.
**Open PRs on DM92:** 0. KP side: `default_branch=chore/dependabot-2026-06-08` (dependabot branch per `gh api repos/<REDACTED>/PhenoContracts --jq '.default_branch'`). The `main` branch is the historical default; the chore branch became default 2026-06-08.
...
```bash
gh auth status
# Expected: "Active account: true" with "<REDACTED>" — Dmouse92 is the read-only collaborator
```
...
- KP-side remains unaffected (`gh api repos/<REDACTED>/<repo> --jq '.archived'`) → `false` for the 11 active repos, `true` for the 2 already-archived ones
...
**File path pattern:** `/Users/<REDACTED>/CodeProjects/Phenotype/repos/worklogs/L5-104-<repo>-archive-2026-06-17.json`
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/PhenoCompose --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/PhenoPlugins --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/PhenoProc --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/HeliosCLI --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/Pyron --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/HexaKit --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/Tracera --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/Civis --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/OmniRoute --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/KWatch --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/phenotype-ops --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/phenotype-otel --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/nanovms --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/PhenoContracts --jq '.archived'",
...
```json
{
  ...
  "verification": {
    "commands": [
      ...
      "gh api repos/<REDACTED>/phenotype-teamcomm --jq '.archived'",
...
```bash
$ gh api "https://api.github.com/repos/<REDACTED>/PhenoProc" --jq '{name, archived, description, updated_at, pushed_at}'
{
  "archived": true,
...
$ gh api "https://api.github.com/repos/<REDACTED>/nanovms" --jq '{name, archived, description, updated_at, pushed_at}'
{
  "archived": true,
...
```bash
$ gh api "https://api.github.com/repos/<REDACTED>/HeliosCLI" --jq '.default_branch'
chore/threat-model-2026-06-16
$ gh api "https://api.github.com/repos/<REDACTED>/PhenoContracts" --jq '.default_branch'
