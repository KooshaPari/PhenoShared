...
**Upstream classification (gh CLI as <REDACTED>):** `missing_or_deleted` — GraphQL "Could not resolve to a Repository".
...
|---|---|---|
| **README identity** | `git show main:README.md` in `004dc7b071f2.git` vs `gh api repos/<REDACTED>/Tokn/contents/README.md` | First line of `Tokn` README: *"Enterprise-grade token management and pricing governance system for AI coding agents."* — byte-identical to `tokenledger`'s tagline. |
...
| **Target present & active** | `gh repo view <REDACTED>/Tokn` | Returns valid metadata; not archived; not private; defaultBranchRef present. |
...
2. ⏭️ Push all eligible branches from `004dc7b071f2.git` to `<REDACTED>/Tokn` as `airlock-recovery/<branch>`. Branches already present on `Tokn` will be skipped. `gh-pages` (if any) will be skipped — none in this mirror.
...
```bash
for branch in $(gh api repos/<REDACTED>/Tokn/branches --paginate \
  ...
  gh api -X DELETE "repos/<REDACTED>/Tokn/git/refs/heads/${branch}" || true
done
```
...
| artifact | location |
|---|---|
| airlock bare mirror | `/Users/<REDACTED>/.airlock/repos/004dc7b071f2.git/` |
