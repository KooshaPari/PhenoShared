...
| **Primary target — `<REDACTED>/bifrost`** | `gh repo view <REDACTED>/bifrost --json createdAt,primaryLanguage,description,repositoryTopics` | Fork created 2026-05-01; description mentions 50×-LiteLLM perf; 1300+ branches; active. The fork has absorbed the routing/SLM/embeddings feature space that `bifrost-extensions` was once a separable layer for. |
| **Alt target — `<REDACTED>/argis-extensions`** | `gh repo view <REDACTED>/argis-extensions` | Description: *"Argis gateway extensions - routing, SLM, embeddings, plugin architecture"* — same extension name pattern; sibling project (Argis is presumably a renamed/renamed gateway family). |
...
2. ⏭️ Push eligible branches from `724f57c187ac.git` to **`<REDACTED>/bifrost`** (primary target) as `airlock-recovery/<branch>`. Branches already on `bifrost` skipped. No `gh-pages` in this mirror.
...
```bash
for branch in $(gh api repos/<REDACTED>/bifrost/branches --paginate \
                 --jq '.[] | select(.name | startswith("airlock-recovery/")) | .name'); do
  echo "deleting ${branch}"
  gh api -X DELETE "repos/<REDACTED>/bifrost/git/refs/heads/${branch}" || true
done
```
...
| artifact | location |
|---|---|
| airlock bare mirror | `/Users/<REDACTED>/.airlock/repos/724f57c187ac.git/` |
