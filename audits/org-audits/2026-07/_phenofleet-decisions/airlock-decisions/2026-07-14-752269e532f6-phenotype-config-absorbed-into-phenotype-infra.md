...
|---|---|---|
| **Smoking-gun commit** | `<REDACTED>/phenotype-infra` commit history | `2026-06-27`: *"fix(crate-deps): repair 3 pre-existing compile failures across nvms-ffi, **pheno-config**, pheno-compose"* — `pheno-config` is now a workspace crate inside `phenotype-infra`. |
| **Absorption wave** | `<REDACTED>/phenotype-infra` commit history | `2026-06-23` × 3 commits: *"A-02: absorb nanovms core"*, *"A-03: absorb PhenoCompose driver"*, *"A-04: absorb BytePort tooling"* — your `phenotype-config` was absorbed in the same wave. |
...
| **NOT a target — disambiguation** | `gh repo view <REDACTED>/pheno-runtime-config` | A *different, newer* repo (hot-reloadable runtime config, separate scope). Don't confuse the two. The absorption target is `phenotype-infra`, not `pheno-runtime-config`. |
...
2. ⏭️ Push eligible branches from `752269e532f6.git` to `<REDACTED>/phenotype-infra` as `airlock-recovery/<branch>`. Branches already on `phenotype-infra` skipped. No `gh-pages` in this mirror.
...
```bash
for branch in $(gh api repos/<REDACTED>/phenotype-infra/branches --paginate \
                 --jq '.[] | select(.name | startswith("airlock-recovery/")) | .name'); do
  echo "deleting ${branch}"
  gh api -X DELETE "repos/<REDACTED>/phenotype-infra/git/refs/heads/${branch}" || true
done
```
...
| artifact | location |
|---|---|
| airlock bare mirror | `/Users/<REDACTED>/.airlock/repos/752269e532f6.git/` |
