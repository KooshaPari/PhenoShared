...
**Evidence:** `gh pr view 130 --repo <REDACTED>/pheno --json title,state,headRefName,baseRefName,additions,deletions,changedFiles`; same for #131, #132.
...
For every file path Dmouse92's ADR-012 branch touches, we queried the GitHub Commits API on `<REDACTED>/pheno` with `path=<file>&since=2026-06-15T00:00:00Z` (the day after the last Dmouse92 ADR-012 commit) to determine which files KP has independently touched in the relevant window.
...
**Evidence:** `bash /tmp/dmouse92-migration/cross-ref3.sh` (uses `gh api repos/<REDACTED>/pheno/commits?path=<file>&since=2026-06-15T00:00:00Z&per_page=10 --jq 'length'`).
...
|---|---|---|---|---|---|
| 1 | `e71a4fd` | tangent | **discard** | Adds `.github/workflows/governance.yml` (1 file); KP/main already has `governance.yml` (added 2026-04-29 per `gh api repos/<REDACTED>/pheno/contents/.github/workflows`). KP already absorbed this. | (none) |
...
| 3 | `f6398a6` | NO | **discard** | Adds `agileplus/crates/phenotype-migrations/src/lib.rs` (350 lines) and `agileplus/Cargo.toml` entry. Lives in `agileplus/` which is KP-owned. The Dmouse92 version is divergent from KP/main; KP team should accept/reject via `<REDACTED>/AgilePlus` directly, not via `pheno`. | (none) |
...
- `9bf8816`: `diff <(git show kp/main:.github/workflows/ci.yml) <(git show chore/adr-012-config-consolidation-2026-06-15:.github/workflows/ci.yml) | grep "uses:" | head` shows identical SHAs (codecov-action@fb8b3582..., commitlint@b948419...) as KP/main. PR #132 history: `gh api 'repos/<REDACTED>/pheno/commits?per_page=10&path=.github/workflows/ci.yml'` line 4 = `10671d2 chore: pin all GitHub Actions to commit SHAs (#132) 2026-05-01`.
...
### 3.1 `<REDACTED>/phenotype-config` (Rust core)

**Repo metadata** (`gh api repos/<REDACTED>/phenotype-config`):
- `default_branch`: `main`
- `archived`: `false`
...
**Recent commits** (`gh api 'repos/<REDACTED>/phenotype-config/commits?per_page=15'`):
- `599d37d 2026-06-17T08:53:56Z feat(genesis): bootstrap config role with settly crate (RFC 002)` — single bootstrap commit.

**Root inventory** (`gh api repos/<REDACTED>/phenotype-config/contents`):
```
...
**Crates layout** (`gh api repos/<REDACTED>/phenotype-config/contents/crates`):
```
dir: settly/
```
...
### 3.2 `<REDACTED>/Conft` (TypeScript edge)

**Repo metadata** (`gh api repos/<REDACTED>/Conft`):
- `default_branch`: `main`
- `archived`: `false`
...
**Root inventory** (`gh api repos/<REDACTED>/Conft/contents`):
```
...
**TS package** (`gh api repos/<REDACTED>/Conft/contents/typescript/packages/conft`):
...
| **Destination** | `<REDACTED>/phenotype-config/docs/slsa.md` |
| **Commit msg** | `feat(docs): port SLSA build attestation doc from Dmouse92 pheno (L5-104)` |
| **Verification** | `gh api repos/<REDACTED>/phenotype-config/contents/docs/slsa.md` returns 200; content matches Dmouse92 source byte-for-byte |
...
| **Destination** | `<REDACTED>/phenotype-config/.github/workflows/release-attestation.yml` |
| **Commit msg** | `feat(ci): port release-attestation workflow from Dmouse92 pheno (L5-104)` |
| **Adapts** | Replace `actions/checkout@<sha>` with substrate-specific SHA (verify via `gh api repos/<REDACTED>/phenotype-config/contents/.github/workflows` first; align with `crates/settly` CI SHAs) |
...
- `forks`: 0
- `parent`: `null` (Dmouse92's fork is not a fork of `<REDACTED>/pheno`)
...
```json
{
  ...
  "inputs": [
    ...
    "<REDACTED>/pheno @ main (PRs #130, #131, #132 reviewed)",
    "<REDACTED>/phenotype-config @ main (genesis commit 599d37d)",
    "<REDACTED>/Conft @ main",
  ...
  "evidence_commands": [
    "gh pr view 130 --repo <REDACTED>/pheno --json title,state,headRefName,baseRefName,additions,deletions,changedFiles",
    "gh pr view 131 --repo <REDACTED>/pheno --json title,state,headRefName,baseRefName,additions,deletions,changedFiles",
    "gh pr view 132 --repo <REDACTED>/pheno --json title,state,headRefName,baseRefName,additions,deletions,changedFiles",
    "gh api repos/<REDACTED>/phenotype-config --jq '{name, default_branch, archived, description, pushed_at}'",
    "gh api repos/<REDACTED>/Conft --jq '{name, default_branch, archived, description, pushed_at}'",
...
- Substrate repos: `gh api repos/<REDACTED>/phenotype-config` + `gh api repos/<REDACTED>/Conft` (retrieved 2026-06-17)
