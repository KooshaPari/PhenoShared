...
3. **GitHub CLI** authenticated with `admin:org` scope **or** a
   ...
   $t = gh api -X POST orgs/<REDACTED>/actions/runners/registration-token --jq .token
   ```
...
> (`actions.runner.<REDACTED>.desktop-<REDACTED>-desk`). The script
...
gh api orgs/<REDACTED>/actions/runners `
  --jq '.runners[] | select(.name == "desktop-<REDACTED>-desk") | {name, status, busy, labels: [.labels[].name]}'
...
jobs:
  gpu-build:
    runs-on: [self-hosted, Windows, X64, desktop-<REDACTED>-desk]
    steps:
