...
>
> | Profila Script | ObservabilityKit Equivalent |
> |---|---|
> | `profiler.sh` (root launcher) | `python/performance_kit/scripts/profiler.py` |
> | `bin/profiler.sh` (system profiling) | `rust/phenotype-health-runtime/` |
...
> | `bin/disk_profiler.sh` | `rust/phenotype-observability-client/` (disk I/O via OTEL) |
> | `bin/network_profiler.sh` | `rust/phenotype-observability-client/` (network metrics via OTEL) |
> | `bin/complexity_analyzer.py` | `python/performance_kit/scripts/analyze_complexity.py` |
> | `bin/generate_charts.py` | `python/performance_kit/scripts/benchmark.py` (chart section) |
> | `bin/continuous_profiler.py` | `python/performance_kit/scripts/profiler.py` + `scripts/duration_tracker.py` |
> | `bin/profiler_setup.sh` | `Taskfile.yml` + `Cargo.toml` (declarative deps) |
