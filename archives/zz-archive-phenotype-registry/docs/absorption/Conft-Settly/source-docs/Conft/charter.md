# Charter — Conft

> **Boundary class:** sdk-domain  
> **Role:** config  
> **Lifecycle:** active  
> **Genesis template:** HexaKit `templates/genesis/` v1.0.0

## Mission

TypeScript configuration edge for the config role (settly companion).

## Scope

### In scope

- TS config loaders and schema surfaces for fleet repos
- Genesis governance for config-role polyglot edges

### Out of scope

| Boundary | Owner repo |
|----------|------------|
| Rust settly core | `phenotype-config` |
| Python config-kit | `phenotype-python-sdk` |

## Governance artifacts

| Artifact | Path |
|----------|------|
| Intent | [intent.md](intent.md) |
| Review | [review.md](review.md) |
| SOTA | [SOTA.md](SOTA.md) |
| OKF | [okf/manifest.okf.yaml](okf/manifest.okf.yaml) |

Authority: [phenotype-registry DOMAIN_ROLES](https://github.com/KooshaPari/phenotype-registry/blob/main/DOMAIN_ROLES.md)

## Decision rights

| Action | Authority |
|--------|-----------|
| Merge to `main` | <REDACTED> + 1 reviewer |

## Changelog

| Date | Change |
|------|--------|
| 2026-06-17 | Genesis rollout Wave 5 |
