# ADR 0020 — Use Phenotype Fabric as a working name only
...
A name is needed for coherent specifications, but branding and repository placement have not been decided by evidence.
...
Use `Phenotype Fabric` / `phenotype-fabric` in this package. Treat it as provisional and avoid irreversible namespace or legal claims until product-boundary validation.
...
- **Alternative 1:** No name at all
- **Alternative 2:** Immediately rename NVMS
- **Alternative 3:** Use ShareCLI brand
- **Alternative 4:** Invent public launch branding
...
- Docs are readable.
- A later ADR may rename without changing IDs.
- Package metadata must mark the name as working.
