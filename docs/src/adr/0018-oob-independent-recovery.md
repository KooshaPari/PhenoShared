# ADR 0018 — Maintain an independent out-of-band recovery plane
...
Software remoting cannot reach BIOS, boot failures, driver crashes, or an unavailable OS.
...
Important physical machines expose KVM-over-IP or physical-switch recovery on a restricted management network. OOB is a separate capability and never treated as the preferred high-performance path.
...
- **Alternative 1:** Software-only recovery
- **Alternative 2:** Publicly expose KVM appliance
- **Alternative 3:** Use OOB for daily high-refresh work
...
- Bench reliability improves.
- Security burden is explicit.
- OOB hardware cost is accepted only for important nodes.
