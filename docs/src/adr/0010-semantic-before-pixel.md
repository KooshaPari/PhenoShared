# ADR 0010 — Prefer semantic application remoting before pixel proxies
...
Per-window pixel projection is broadly compatible but loses window, text, accessibility, and protected-surface semantics.
...
Use RAIL/RemoteApp, Xpra, Waypipe, Qubes/WSLg-like semantic paths where available. Use a native pixel proxy as a universal fallback, then whole desktop when proxy semantics fail.
...
- **Alternative 1:** Always stream full desktop
- **Alternative 2:** Always pixel-capture each window
- **Alternative 3:** Require app-native web UI
- **Alternative 4:** Claim process migration
...
- Best available semantics are retained.
- The proxy implementation still requires owned-window graphs, IME, DPI, and atlas scaling.
- Fallback is explicit rather than hidden.
