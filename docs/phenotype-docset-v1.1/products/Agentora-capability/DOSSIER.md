# Agentora-capability: role-specific atlas and qualification

**Role:** Agent framework API after absorption. **Class:** absorbed-capability. **Repository ID:** Not independently resolved; capability-level identity required.
...
Inventory the actual exported Rust/other framework, tools, state, model adapters, storage and consumers. Locate legacy Agentora code by lineage rather than an agentkit string alone.
...
Compare direct SDK, OpenAI Agents SDK, LangGraph, CrewAI, Mastra, PydanticAI and Rig; prefer a maintained foundation where it preserves the valuable contract.
...
A matched coding-agent pilot proves correctness, ergonomics and recovery; retained custom behavior has a defensible measured benefit or internal boundary.
