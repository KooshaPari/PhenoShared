...
The names `phenotype-py-utils` and `phenotype-ts-utils` were suggested as utility absorption targets, but they do not currently resolve under `<REDACTED>` via `gh repo list` or direct clone. Do not block source repo deletion readiness on these names.

## Resolved targets

| Suggested target | Resolution evidence | Status | Required action |
|---|---|---|---|
| `phenoUtils` | `<REDACTED>/phenoUtils` exists and is local; contains Rust crates `pheno-crypto`, `pheno-fs`, `pheno-net`, `pheno-shell`, `pheno-testing` | usable Rust primitive target | use only for true Rust primitive utilities |
| `phenotype-py-utils` | direct clone failed; `gh repo list <REDACTED>` utility search did not return it | `NOT_COVERED` / non-existent target | use `phenotype-python-sdk` packages for Python utilities unless a repo is created intentionally |
| `phenotype-ts-utils` | direct clone failed; `gh repo list <REDACTED>` utility search did not return it | `NOT_COVERED` / non-existent target | use a real TS owner or create a tight repo intentionally before migration |
