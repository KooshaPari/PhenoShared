...
`2phenoEvents` is a historical recovery alias, not an active repository and not
an absorption candidate. The live remote is not resolvable. Its recorded source
commit is reachable as the root restore commit in canonical `phenoEvents`, so
there is no unique source slice to move.

This tombstone preserves lineage without asserting that the alias never existed.
It authorizes no deletion, archive action, branch mutation, or source transfer.
