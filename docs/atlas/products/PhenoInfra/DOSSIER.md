# PhenoInfra: role-specific atlas and qualification

**Role:** Compute/runtime and operational infrastructure. **Class:** pooled-foundation. **Repository ID:** 1220334053.
...
Recover runtime/provisioner, manifest, secrets, state, backup/restore and consumer contracts. Separate reusable libraries from environment-specific resources.
...
Use supported provider/runtime APIs, existing reconciliation tools and verified backup software. Do not establish a new cluster to satisfy a GitOps label.
...
A real clean consumer provisions and recovers its intended resource, and a restore drill preserves state and ownership.
